...
def complex_source_point(config_source):
    ...
    theta_file = np.loadtxt('./inputs/angle_elev.csv',delimiter=',')
    G_file = np.loadtxt('./inputs/DME_gain.csv',delimiter=',')
    #theta_file = np.fromiter(theta_file.values(), dtype=float)
    #G_file = np.fromiter(G_file.values(), dtype=float)
    size_G_file = np.size(G_file)
    # ------------------------- #
    ...
    elevation = np.array(np.arctan((z_vect-z_s)/(-x_s))) #rad
    G = np.zeros(n_z)
    for ii_z in range(n_z):
        theta = elevation[ii_z]
        diff_theta = np.abs(theta_file - theta*180/np.pi)
        index = np.argmin(diff_theta)
        G[ii_z] = G_file[index]

    e_field = np.sqrt(zeta0 * config_source.P_Tx * 10**(G/10) / (2*np.pi)) / (np.sqrt(x_s**2 + (z_vect-z_s)**2))*np.exp(-1j * k0*np.sqrt(x_s**2 + (z_vect-z_s)**2))
    # ------------ END --------------- #
    # --- Computation of the field --- #
    # -------------------------------- #
