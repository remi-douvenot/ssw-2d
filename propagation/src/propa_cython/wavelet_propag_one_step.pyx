...
# wv_x structure is [A(J) D(J) D(J-1) ..... D(1)]
# where A(J) is the approximation coefficient vector at the Jth level while D(n) are the detail coefficient vectors
# library structure is [p(0) p(1) p(n) ..... p(2**L)]
...
def wavelet_propag_one_step_cy(const int n_z, const double complex[:] wv_x, double complex[:] library, const int wv_ll,
    ...
    cdef Py_ssize_t n_start_propa_lvl, n_stop_propa_lvl  # where index of propagators of current level begins / ends in "library"
    cdef Py_ssize_t n_start_propa_lvl_q, n_stop_propa_lvl_q  # where index of current propagator begins / ends in "library"
