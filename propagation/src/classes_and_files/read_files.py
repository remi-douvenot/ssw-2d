...
from propagation.src.classes_and_files.classes import *
import csv
import numpy as np
...
def read_config(file_configuration):
    ...
    for row in file_tmp:
        if row[0] == 'method':  # SSF or SSW or WWP or WWP-H
            ...
            sigma = np.float64(row[1])  # ground conductivity                                                         change
    ...
    if (Config.method != 'SSW') & (Config.method != 'WWP') & (Config.method != 'WWP-H') & (Config.method != 'SSF') & (Config.method != 'WGM'):
        raise ValueError(['Method must be chosen among: SSW or WWP or WWP-H or SSF or WGM'])
    # ------------ END ------------ #
    # --- Reading configuration --- #
    # ----------------------------- #
