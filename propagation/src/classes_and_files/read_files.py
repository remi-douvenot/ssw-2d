    ...
    with open(file_configuration, newline='') as f_config:
        ...
        for row in file_tmp:
            if row[0] == 'method':  # SSF or SSW or WWP or WWP-H
                ...
                config.z_step = np.float64(row[1])
            elif row[0] == 'frequency':
                config.freq = np.float64(row[1]) * 1e6  # freq in MHz
            elif row[0] == 'polarisation':
                config.polar = row[1]  # 'TE' or 'TM'
            elif row[0] == 'Max compression error':
                config.max_compression_err = np.float64(row[1])  # Max compression error
                ...
                config.apo_z = np.float64(row[1])  # apodisation size along z
            elif row[0] == 'image size':
                config.image_layer = np.float64(row[1])  # image layer size (in ground) along z
            elif row[0] == 'ground':
                config.ground = row[1]  # ground type
            elif row[0] == 'epsr':
                config.epsr = np.float64(row[1])  # ground relative permittivity
            elif row[0] == 'sigma':
                sigma = np.float64(row[1])  # ground conductivity
            elif row[0] == 'atmosphere':
                config.atmosphere = row[1]  # atmospheric profile type
            elif row[0] == 'c0':
                config.c0 = np.float64(row[1])  # standard atm gradient
            elif row[0] == 'delta':
                config.delta = np.float64(row[1])  # evaporation duct height
            elif row[0] == 'zb':
                config.zb = np.float64(row[1])  # base height of a trilinear duct
            elif row[0] == 'c2':
                config.c2 = np.float64(row[1])  # gradient in a trilinear duct
            elif row[0] == 'zt':
                config.zt = np.float64(row[1])  # thickness of a trilinear duct
                ...
                config.Cn2 = np.float64(row[1])
            elif row[0] == 'L0':
                config.L0 = np.float64(row[1])
            elif row[0] == 'py_or_cy':
    ...
    with open(file_source_config, newline='') as f_source_config:
        ...
        for row in file_tmp:
            ...
            # z_s is the source altitude in the z direction
                ...
                z_s = np.float64(row[1])  # altitude in m                                                            change
    ...
    with open(file_relief_config, newline='') as f_source_config:
        ...
        for row in file_tmp:
            ...
            # geometry must match with the source generation
                ...
                x_step = np.float64(row[1])  # horizontal step in m
            elif row[0] == 'z_max_relief':
                z_max_relief = np.float64(row[1])  # max relief in m
