...
import src.atmosphere.anaprop.atmosphere as atm
import src.atmosphere.anaprop.relief as rel
import src.atmosphere.anaprop.cds as cds
...
def get_corefractive_index(case:Case, N_x:int, N_z:int, z_step:float, data_path='./', weather_source='era5'):
    if weather_source == "era5":
        path = cds.download_dataset(case.when, data_path)
    else:
        raise ValueError(f"Unsupported weather source \"{weather_source}\"")
    ...
    # Load initial dataset
    a = atm.load(path, case.P, case.Q, N_x)
    # Normalize vars and coords names
    a = atm.normalize(a)
    # Retrieve the height profile from IGN 
    h = rel.get_bing_height_profile(case.P, case.Q, N_x)
    # Compute the refraction index
    a = atm.add_n(a)
    a = atm.add_heights(a)
    a = atm.add_M(a)
    ...
    a = atm.add_grad_M(a)
    # Interpolate all the dataset in between pressure levels, and convert pressure levels to meters
    a = atm.interpolate_vertical(a, N_z, hmax)
    # Remove the data that is below the height profile
    a = a.where(a.heights > h.height)
    # Replace NaNs with 1
    a = a.fillna(1.0)
    # Add m
    a = a.assign(m = lambda ds: 1 + ds.M * 1e-6)
    # Create time string used to select data
    time = case.when.strftime("%Y-%m-%dT%H:00:00.%f")
    # Export terrain
    np.savetxt("../terrain/outputs/z_relief.csv", np.concatenate([[0.0], np.array(h.height)]))
    return np.array(a.m.sel(time=time)).T # transpose the array, shape of (N_x, N_z) instead of (N_z, N_x)
